...
usage() {
	printf 'Usage: install-signed-packages.sh [--check-only] SIGNED_PACKAGE_DIR CERTIFICATE\n'
}
...
for command in dpkg-deb find modinfo nproc openssl realpath sbverify sed sort tr wc xargs; do
